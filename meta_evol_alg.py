...
# Evolve
def evolutionary_algorithm(mut_rate,mut_size,season_len,truncation_prop, pop_size):
    args = FakeArgs()
    ...
    for gen in range(args.num_generations):
        ...
        # MUTATION
        num_genes_mutate = max(int(args.grn_size*args.grn_size*len(children) * args.mut_rate),1)
...
# season length [1, 1000]
# truncation    [0.01, 1]
# mut_rate      [0,    1]
# mut_size      [0,    1]

def get_truncation_sizes(pop_size):
  return np.array([i/pop_size for i in range(1,pop_size//2 - 1) if pop_size % i == 0])

NGENES = 4
POPSIZE = 20
INNERPOPSIZE = 1000
truncation_sizes = get_truncation_sizes(INNERPOPSIZE)
TSIZES = len(truncation_sizes)

@njit("f8[:](f8[:])")
def mutate(x):
  x1 = np.copy(x)
  # order: mut_rate, mut_size, season_len, truncation_prop
  x1[0:3] += np.random.randn(3) *0.001
  x1[0:3] = np.clip(x1[0:3], 0.01 , 1.) 
  x1[3] = truncation_sizes[np.random.randint(TSIZES)]
  return x1
...
def score(pop):
  ...
  for i,ind in enumerate(pop):
    fitnesses[i] = evolutionary_algorithm(*ind, pop_size=INNERPOPSIZE)
  #print(fitnesses)
  return fitnesses

#::::@njit("f8[:,:](f8[:,:], f8[:], i8, i8, i8)")
def step(pop, fitnesses, P, K, O):
    """
    ...
    #assert fitnesses[fit_idxs].mean() >= fitnesses[unfit_idxs].mean(), f"{fitnesses[fit_idxs].mean()} < {fitnesses[unfit_idxs].mean()} : {unfit_idxs}"
    top_k = pop[fit_idxs]
    children = np.empty(((P - K), pop.shape[1]))

    ##### MUTATION ######
    for i in range(P - K):
        j = i % K
        children[i] = mutate(top_k[j])
...
def evolve(its):
  # season length [0.01, 1] * 1000 INT
  # truncation    [0.01,    1] FLOAT
  # mut_rate      [0.01,    1] FLOAT
  # mut_size      [0.01,    1] FLOAT
  # order: mut_rate, mut_size, season_len, truncation_prop

    pop = np.random.uniform(low=0.01, high=1., size=(POPSIZE, NGENES))
    pop[:,3] = np.random.choice(truncation_sizes, size=(POPSIZE))

    K = 5 #number of parents
    O = 2 #fraction of crossovers, 2=half of indvs, because every 2nd
    ...
    for _ in pbar:
        fitnesses = score(pop)
        pbar.set_description(f"{np.max(fitnesses):.5f}")
        bests.append(np.copy(pop[np.argmax(fitnesses)]))
        myfits.append(np.max(fitnesses))
        pop = step(pop, fitnesses, POPSIZE, K, O) #update pop (selection, mutation, crossover)

    return bests, myfits

bests, myfits = evolve(its=10)