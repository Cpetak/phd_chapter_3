...
def evolutionary_algorithm(args, title, folder):
    ...
    for gen in trange(args.num_generations):
        ...
        mut_parent = parents.repeat([1, 1, 1])
        num_genes_mutate = int(args.grn_size*args.grn_size*len(mut_parent) * args.mut_rate)
        mylist = torch.zeros(args.grn_size*args.grn_size*len(mut_parent), device=device)
        mylist[:num_genes_mutate] = 1
        shuffled_idx = torch.randperm(args.grn_size*args.grn_size*len(mut_parent), device=device)
        mask = mylist[shuffled_idx].reshape(len(mut_parent),args.grn_size,args.grn_size) #select genes to mutate
        mut_parent = mut_parent + (mut_parent*mask)*torch.randn(size=mut_parent.shape, device=device) * args.mut_size  # mutate only children only at certain genes
        
        pop[parent_locs] = mut_parent # put children into population
...
if __name__ == "__main__":
    ...
    parser.add_argument('-exp_type', type=str, default="no_parents", help="Name your experiment for grouping")
