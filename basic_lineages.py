...
def evolutionary_algorithm(args, title, folder):
    ...
    IDs=torch.zeros((args.num_generations+1,args.pop_size,2),dtype=torch.int).to(device) #keep track of parent's id and your id
    IDs[0,:,1]=torch.arange(0, args.pop_size)

    num_possible_ids=args.pop_size + ((args.pop_size-args.truncation_size)*args.num_generations)
    id_stds=torch.ones(num_possible_ids).to(device) #keep track of id stds
    id_stds=id_stds*-1
    id_fits=torch.ones(num_possible_ids).to(device) #keep track of id fits
    ...
    for gen in trange(args.num_generations):
        ...
        if gen > 0:
          ...
          id_stds[[parent_IDs.tolist()]] = stds.mean(1)

        # Evaluate fitnesses 
        fitnesses = fitness_function(phenos, targs[curr_targ])
        if gen > 0:
          id_fits[[parent_IDs.tolist()]] = fitnesses[parent_locs]
          id_fits[[kid_IDs.tolist()]] = fitnesses[children_locs]
        ...
        parent_IDs=IDs[gen,parent_locs,1]
        children_IDs = parent_IDs.repeat([num_child])  
        ...
        IDs[gen+1,parent_locs,0] = parent_IDs #keeping track of where they are coming from
        IDs[gen+1,children_locs,0] = children_IDs #keeping track of where they are coming from
        IDs[gen+1,:,1][parent_locs] = parent_IDs #parents stay the same ID
        id_min=args.pop_size+len(children_locs)*gen
        id_max=id_min+len(children_locs)
        kid_IDs = torch.arange(id_min, id_max, dtype=torch.int).to(device) #assign new ids to new individuals
        IDs[gen+1,:,1][children_locs] = kid_IDs
    ...
    #IDs, id_stds, id_fits
    stats["IDs"] = IDs
    stats["id_stds"] = id_stds
    stats["id_fits"] = id_fits
